function [opt,teResults,mseResults] = Step2_D_train_cv(traindata,testdata,trainlabel,testlabel,opt,currentK,currentDocument)
%currentK当前k折
%currentDocument当前文档
...
%                    for g = 1:1:1
                ...
                disp([currentDocument,'  ',opt.algorithm,'  ',num2str(currentK),'折  ',num2str((i-1)*j+j),'/',num2str(sumlength),'   ',num2str((c-1)*length(opt.g)+g),'/',num2str(length(opt.g)*length(opt.c)),'/',num2str(length(opt.g)*length(opt.c)),'次']);
                ...
                for k = 1:opt.cvk
                    ...
                    switch opt.algorithm
                        ...
                        case 'LR'
                            ...
                            S = outA * outB;
                        case 'RSR'
                            [S,~] = L21R21(traindata(train,:),trainlabel(train,:),1);
                        case 'xijAB_ABS'
                       ...
                       case 'jelsr'
                           %  opt.classnum换为6
                            ...
                            [W1,~,~] = jelsr(traindata(train,:),W,6,1,1);
                            S = W1;
                    case 'RPCA_OM'
                            %  opt.classnum换为6
                    ...
                    case 'FRFS0'
                        %% FRFS0  zhupengfei:Non-convex Regularized Self-representation for Unsupervised Feature Selection
                            [S,obj] = FRFS0(traindata(train,:),1,10);
