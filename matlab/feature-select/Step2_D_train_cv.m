function [opt,pred_label,teResults,mseResults] = Step2_D_train_cv(traindata,testdata,trainlabel,testlabel,opt,currentK,currentDocument)
%currentK当前k折
%currentDocument当前文档
...
%                    for g = 1:1:1
                ...
                disp([currentDocument,'  ',opt.algorithm,'  ',num2str(currentK),'折  ',num2str((i-1)*j+j),'/',num2str(sumlength),'   ',num2str((c-1)*length(opt.g)+g),'次']);
                ...
                for k = 1:opt.cvk
                    ...
                    case 'RPCA_OM'
                            %  opt.classnum换为6
                            [W1,~,~] = RPCA_OM(traindata(train,:)',1,20);
                            S = W1;
                    ...
                    case 'FSSI'
                     ...
                     case 'SRFS_WS'
                         ...
                         lambda2 = 10;
                         lambda3 = 100;
                         [W, ~, ~] = SRFS_WS(traindata(train,:),trainlabel(train,:),lambda1,lambda2,lambda3);
                          S = W;
                    end
                    %算法调用 end 
...
switch opt.algorithm
   ...
   case 'jelsr'
    ...
    case 'SRFS_WS'
         ...
         lambda2 = 10;
         lambda3 = 100;
         [W, ~, ~] = SRFS_WS(traindata, trainlabel,lambda1,lambda2,lambda3);
         S = W;
end
%%算法调用 end 
