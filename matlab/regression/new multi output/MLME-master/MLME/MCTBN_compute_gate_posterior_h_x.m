function [ h_x ] = MCTBN_compute_gate_posterior_h_x(Experts, gate, X, Y, is_switching)
...
if is_parallelized
    h_x = MCTBN_compute_gate_h_x_parallel( Experts, gate, X, Y, is_switching );
else
    h_x = MCTBN_compute_gate_h_x_sequential( Experts, gate, X, Y, is_switching );
end
...
function [ h_x ] = MCTBN_compute_gate_h_x_sequential(Experts, gate, X, Y, is_switching)
...
function [ h_x ] = MCTBN_compute_gate_h_x_parallel(Experts, gate, X, Y, is_switching)
...
parfor j = 1:num_batches
    tmp_h_x{j} = MCTBN_compute_gate_h_x_sequential(Experts, gate, X, Y, is_switching);
end
