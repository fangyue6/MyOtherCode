...
addpath(genpath([currentFolder,'\MLME-master']));
...
for    j=1:length(lambda)
    ...
    for i = 1:10
       ...
       %% YYQ_regression
        ...
        para_YYQ.beta = 2;
        para_YYQ.p = 1.5;
        [W,~] = YYQ_regression(X_tr,Y_tr,para_YYQ);
        normYYQ = sqrt(sum(W.*W,2));
        [YYQ_Weight, result_YYQ_sorted_features] = sort(-normYYQ);
        para_YYQ.percent =1;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Num_SelectFearesult_YYQ = floor(para_YYQ.percent*d);       
        result_YYQ_SelectFeaIdx{i} = result_YYQ_sorted_features(1:Num_SelectFearesult_YYQ,:);
        result_YYQXTrain = X_tr(:,result_YYQ_SelectFeaIdx{i});
        result_YYQXTest = X_te(:,result_YYQ_SelectFeaIdx{i});
        [result_YYQ.aCC(j,i),result_YYQ.aRE(j,i),result_YYQ.MSE(j,i),result_YYQ.aRMSE(j,i),result_YYQ.aRRMSE(j,i),result_YYQ.MAE(j,i)] =...
        DD_SVR_By_FY(Y_tr,Y_te,result_YYQXTrain,result_YYQXTest);%aCC, aRE, MSE, aRMSE, aRRMSE, MAE
...
%      %%  MLME_CCs2
%         CC = [];
% %         Y_tr = changeNumber(Y_tr)-1;
%         CCY_tr = Y_tr;
%         meanCC = mean(mean(Y_tr));
%         CCY_tr(CCY_tr>meanCC)=1;
%         CCY_tr(CCY_tr<=meanCC)=0;
%         
% 
...
%         CCX_tr = X_tr(:,1:d);
%         CCY_tr = CCY_tr;
%         [CC.model, CC.permutation] = CCs2_train_weighted(CCX_tr, CCY_tr , ones(length(CCY_tr),1));
%         
%         CCmodel =  CC.model;
%         mind =size(CCmodel{1},2);
%         CCW=[];
%         for ci=1:length(CCmodel)
%             CCa=CCmodel{ci};
%             CCW=[CCW;CCa(1:mind)]
%         end
%         WCC = CCW';
...
%         
%         normW_CCs2 = sqrt(sum(WCC.*WCC,2));
%         [W_CCs2_Weight, W_CCs2_sorted_features] = sort(-normW_CCs2);
%         para_CCs2.percent = 0.9;
%         Num_SelectFea_CCs2 = floor(para_CCs2.percent*d);       
%         W_CCs2_SelectFeaIdx{i} = W_CCs2_sorted_features(1:Num_SelectFea_CCs2,:);
%         CCs2XTrain = CCX_tr(:,W_CCs2_SelectFeaIdx{i});
%         CCs2XTest = X_te(:,W_CCs2_SelectFeaIdx{i});
%         [result_CCs2.aCC(j,i),result_CCs2.aRE(j,i),result_CCs2.MSE(j,i),result_CCs2.aRMSE(j,i),result_CCs2.aRRMSE(j,i),result_CCs2.MAE(j,i)] ...
%         =DD_SVR_By_FY(Y_tr,Y_te,CCs2XTrain,CCs2XTest);%aCC, aRE, MSE, aRMSE, aRRMSE, MAE
...
% meanvalue_CCs2 = meanEvaluate_by_FY(result_CCs2)
% save(['result/CCs2/',dataname,'_',num2str(meanvalue_CCs2(1)),'.mat'],'result_CCs2','para_CCs2','meanvalue_CCs2'); 

meanvalue_YYQ = meanEvaluate_by_FY(result_YYQ)
save(['result/YYQ/',dataname,'_',num2str(meanvalue_YYQ(1)),'.mat'],'result_YYQ','para_YYQ','meanvalue_YYQ'); 
