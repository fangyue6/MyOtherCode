...
%'andro','ATP1d','EDM','enb','jura','OES10','osales','scm1d_test','scpf','sf2','slump','WQ'
datasets ={'scm1d_test','scpf','sf2'};
for idata=1:length(datasets)
dataname= datasets{idata};
% load('EDM.mat');
% load('oes97.mat');
...
%         %% MIMLfast
%         for im=1:length(X_tr)
%             MIML_train_data{im,1} = X_tr(im,:);
%         end
%         for ij=1:length(Y_tr)
...
%         CTBN = learn_weighted_structure(X_tr, CCY_tr, ones(length(Y_tr),1), []);
